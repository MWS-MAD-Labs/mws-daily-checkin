...
export function useSilentHubRelogin() {
    ...
    useEffect(() => {
        ...
        getHubBaseUrl().then((hubBaseUrl) => {
            ...
            // here) - that fired just as often for someone opening a
            // shared daily-checkin link cold as it did for a genuinely
            // expired session, bouncing a first-time visitor away from the
            // very link they clicked a few seconds after landing. Silent
            // recovery failing just means this app's own landing page
            // (HeroSection/HeroAuthCard) shows its "Sign in with Hub"
            // button instead - a real logout still reaches Hub for real,
            // that's authService.js's logout(), unrelated to this hook.
