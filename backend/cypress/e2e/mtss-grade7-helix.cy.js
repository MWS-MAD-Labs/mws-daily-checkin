    ...
    describe('Phase 2: Teacher Dashboard Verification', () => {
        beforeEach(() => {
            ...
            cy.request('POST', `${API_BASE_URL}/auth/login`, {
                ...
                window.localStorage.setItem('daily.auth_token', token);
                window.localStorage.setItem('daily.auth_user', JSON.stringify(response.body.data.user));
            });
        });
    ...
    describe('Phase 3: Progress Update Verification', () => {
        ...
        beforeEach(() => {
            ...
            cy.request('POST', `${API_BASE_URL}/auth/login`, {
                ...
                window.localStorage.setItem('daily.auth_token', authToken);
                window.localStorage.setItem('daily.auth_user', JSON.stringify(response.body.data.user));
            });
        });
