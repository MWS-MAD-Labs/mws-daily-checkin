...
const setAuthInBrowser = (token, user) => {
    cy.window().then((win) => {
        win.localStorage.setItem('daily.auth_token', token);
        win.localStorage.setItem('daily.auth_user', JSON.stringify(user));
    });
};
    ...
    describe('Phase 3: Student Profile Page', () => {
        ...
        it('should load student profile page via frontend', () => {
            ...
            cy.visit(FRONTEND_URL, {
                onBeforeLoad(win) {
                    win.localStorage.setItem('daily.auth_token', studentToken);
                    win.localStorage.setItem('daily.auth_user', JSON.stringify(studentUser));
                }
            });
    ...
    describe('Phase 8: Teacher Dashboard - Student Check-ins', () => {
        ...
        it('should load teacher emotional check-in dashboard page (Frontend)', () => {
            cy.visit(`${FRONTEND_URL}/emotional-checkin/teacher-dashboard`, {
                onBeforeLoad(win) {
                    win.localStorage.setItem('daily.auth_token', teacherToken);
                    win.localStorage.setItem('daily.auth_user', JSON.stringify(teacherUser));
                }
            });
