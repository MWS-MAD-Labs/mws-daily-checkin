...
const requiredVariables = ['GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET', 'GOOGLE_REDIRECT_URL'];
const missingVariables = requiredVariables.filter((key) => !process.env[key]);
const googleOAuthConfigured = missingVariables.length === 0;
...
async function googleOAuthVerify(accessToken, refreshToken, profile, done) {
    try {
        ...
        try {
            ...
            console.error('mws-data-center student lookup failed, falling back to staff check:', error.message);
        }
...
if (googleOAuthConfigured) {
    ...
    console.warn('Google OAuth is not configured. OAuth routes will stay unavailable until credentials are provided.');
}
...
passport.googleOAuthStatus = {
    missingVariables,
    callbackURL: process.env.GOOGLE_REDIRECT_URL || null
};
