...
const aiChatProxyRoutes = require('./aiChatProxy');
...
router.use('/v1/ai-chat', aiChatProxyRoutes);

// OAuth routes are now mounted directly in app.js
// router.use('/auth', authRoutes);

// Client-side audit event sink (docx downloads, exports, etc.)
router.post('/v1/audit/events', (req, res) => {
    try {
        ...
        console.info('[AuditLog]', JSON.stringify({
            event, filename, reportTitle, sourcePage,
            actorId, actorName, actorRole, timestamp,
...
router.get('/health', (req, res) => {
    res.status(200).json({
        success: true,
        message: 'MWS Daily Check-In Backend API is running',
